...
// Append nodes A, B, and C to the linked list
list.appendNode('A'); // List: A
list.appendNode('B'); // List: A -> B
list.appendNode('C'); // List: A -> B -> C

// Insert a new node 'D' at position 1
list.insertAt(1, 'D'); // List: A -> D -> B -> C

// Remove the node at position 2
let removedNode = list.removeAt(2); // Removed node: B, List: A -> D -> C

// Search for the position of the node with data 'C'
let position = list.search('C'); // Output: 2 (position of node with data 'C')
...
function printList() {
  ...
  console.log(output.slice(0, -4)); // Remove the last ' -> ' and print
}

// Call the printList function
printList(); // Output: A -> D -> C
